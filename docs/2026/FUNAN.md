...
- **Recently**, I released a Nextflow pipeline for eukaryotic gene prediction and functional annotation called [**GeneForge**](https://github.com/SequAna-Ukon/GeneForge). The pipeline is designed for comprehensive structural and functional annotation of eukaryotic genomes. It orchestrates the parallel execution of BRAKER3 and Funannotate, evaluates their performance using BUSCO, and provides a unified functional annotation workflow. The pipeline does not require local installation, making it easy to use. Feel free to try it out, and please cite it if you use it in your work.
